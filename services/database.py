...
def _normalizar_colunas_inteiras(df: pd.DataFrame) -> pd.DataFrame:
    """
    Converte colunas float64 cujos valores não-nulos são todos "inteiros
    disfarçados" (ex: 1.0, 2.0, 3.0) para o tipo Int64 (inteiro anulável do
    pandas).

    Motivo: quando uma coluna numérica do Excel tem pelo menos uma célula
    vazia, o pandas é obrigado a lê-la como float64 (não existe NaN em
    int64 nativo). Isso faz o valor 1 virar 1.0 e, ao gerar o CSV para o
    COPY, o texto exportado fica "1.0" em vez de "1". Se a coluna de
    destino no Postgres já foi criada como bigint/integer (em uma execução
    anterior em que não havia valores vazios), o COPY falha com:

        invalid input syntax for type bigint: "1.0"

    Convertendo para Int64 aqui, o valor é serializado como "1" (e a
    célula vazia como "" / NULL), compatível com colunas inteiras.
    """
    df = df.copy()
    for col in df.columns:
        if pd.api.types.is_float_dtype(df[col]):
            valores_nao_nulos = df[col].dropna()
            if not valores_nao_nulos.empty and (valores_nao_nulos % 1 == 0).all():
...
def enviar_dados_para_neon(df: pd.DataFrame, nome_tabela: str, coluna_chave: str = "numero_atividade",
    ...
    # Corrige colunas que deveriam ser inteiras mas vieram como float64 por
    # causa de células vazias no Excel (evita erro "invalid input syntax for
    # type bigint" no COPY — veja docstring de _normalizar_colunas_inteiras).
    df = _normalizar_colunas_inteiras(df)
