...
from components.charts import grafico_ranking, grafico_atribuicao_pu, opcoes_grafico
...
def render(df, indicadores: Indicadores):
    ...
    with area_com_print("coordenadores_cards_resumo", nome_arquivo="resumo_coordenadores"):
    ...
    with col_graf:
        with area_com_print("coordenadores_grafico_ranking", nome_arquivo="top15_concluido_coordenadores"):
            st.plotly_chart(
                grafico_ranking(ranking, "Concluído OK", "Top 15 — Concluído OK"),
                width='stretch',
                config=opcoes_grafico("top15_concluido_coordenadores"),
            )

    with col_tab:
        tabela = ranking.sort_values("Concluído OK", ascending=False).copy()
        tabela["Eficácia"] = tabela["Eficácia"] * 100
        with area_com_print("coordenadores_tabela_ranking", nome_arquivo="ranking_coordenadores"):
            st.dataframe(
                tabela,
                width='stretch',
                hide_index=True,
    ...
    with area_com_print("coordenadores_grafico_atribuicao_pu", nome_arquivo="atribuicao_x_pu_coordenadores"):
        st.plotly_chart(grafico_atribuicao_pu(df), width='stretch', config=opcoes_grafico("atribuicao_x_pu_coordenadores"))
    ...
    with area_com_print("coordenadores_matriz_producao", nome_arquivo="producao_por_coordenador"):
